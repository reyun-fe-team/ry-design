...
<template>
  <div :class="classes">
    <!-- 面包屑 -->
    <div v-if="isPropBreadcrumb || isHasSolt('breadcrumb')"
    ...
    <div :class="[prefixCls + '-heading']">
      ...
      <div :class="[prefixCls + '-heading-left']">
        <!-- 返回按钮 -->
        <div v-if="backIcon || isHasSolt('backIcon')"
        ...
        <!-- 头像 -->
        <span v-if="isPropAvatar || isHasSolt('avatar')"
        ...
        <!-- 标题 -->
        <span v-if="title || isHasSolt('title')"
              :class="[prefixCls + '-heading-title']">
          <slot name="title">{{ title }}</slot>
        </span>
        <!-- 副标题 -->
        <span v-if="subTitle || isHasSolt('subTitle')"
