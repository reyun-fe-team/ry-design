<template>
  <main>
    <rd-emoji-image-input
        :poptip-class="'emojiContent'">
      <div slot="input-content"
          class="emoji-input"
          :style="{border: emojiInfoList.length !== totalNum ? '1px solid #ed4014' : ''}">
        <div
      ...
      <div slot="emoji-panel" class="emoji-panel-wrap">
        <img
            v-for="(item,index) in emojiList"
            :key="index"
            class="emoji-item-common"
            :src="item.emojiUrl"
            :emoji-code="item.emojiCode"
            @click="appendEmoji(item)">
      </div>

      <span slot="emoji-input-num" class="emoji-input-num">
  ...
  methods: {
    changeEmoji() {
      if (!this.value.length) {
        this.emojiInfoList.splice(this.emojiInfoList.length - 1, 1)
      }
    },
    appendEmoji(ev) {
      if (this.emojiInfoList.length >= this.totalNum) {
        this.$Message.error(`表情不能超过${this.totalNum}个，请检查！`)
        return
      }
      for (let i = 0; i < this.emojiInfoList.length; i++) {
        if (this.emojiInfoList[i].emojiCode === ev.emojiCode) {
          this.$Message.error(`表情需选择${this.totalNum}个不同表情，请检查！`)
          return
        }
      }
      this.emojiInfoList.push({
        emojiCode: ev.emojiCode,
        emojiUrl: ev.emojiUrl
      })
      let arr = []
      this.emojiInfoList.forEach(item => {
        if (item.emojiCode) {
          arr.push({
...
.emoji-input:focus:before {
  content: none;
}

/deep/ .rd-emoji-image-input-content {
  display: flex;
}
</style>
<style lang="less">
